	...
	<div>
		<b-container>
			<b-table class="tabla" dark striped :items="this.items" caption-top>
				<template slot="table-caption">Monitor de Impugnaciones</template>
				...
				<template slot="pendientes" slot-scope="data">
					...
					<b-button v-if="data.value > 0" :to="{ name: 'ImpugnacionesPendientes', params: { zona: data.item.zona } }"  variant="danger" style="width: 100%;" size="lg">{{ data.value }}</b-button>
	...
	export default {
		...
		methods: {
			...
			getData: function(){
				...
				axios({
					method: 'POST',
					url: 'https://project2019.club/api_proyectojunio/public/impugnaciones_region',
					headers: {
